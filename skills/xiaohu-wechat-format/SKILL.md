...
**分析维度**：文章类型（访谈/教程/产品介绍/深度分析/连环画）、内容元素（对话/图片/代码/数据）、节奏感（密集段 vs 留白段）。
...
3. **连环画/配图文章** → 单张 `<figure>` 纵向展示
   - 3张以上配图且每张附 caption → 用 `<figure><img/><figcaption>` 逐张展示
   - 图片全宽、高度固定（微信会自动压缩为屏幕宽度）
   - 不并排、不用 gallery，每个图片占一整块，后面跟图说
   - 章节间可用 `## 章节标题` 分类
   - 适合：漫画、连环画、图文故事、配图小说
...
| 连环画/配图故事 | chinese, ink, terracotta |
...
- **连环画处理**：`<figure><img/><figcaption>caption</figcaption></figure>` 逐张纵向展示，全宽显示，图片间不并排
